...
#code taken from 
#https://github.com/EverythingSmartHome/mlx90640-thermal-camera/blob/master/thermal-camera-advanced.py
#I have made some modifications for it to print info
...
def plot_update():
    ...
    max_array.append(np.round(np.max(data_array), 1)) #save the highest temp of the array
...
def test_avg_temp():
    ...
    while (wait < 11):
        ...
        wait += 1
    avg = round(sum(max_array)/len(max_array), 1)
